...
from .matchers import LexicalMatcher, Matcher, SemanticMatcher
...
def run_cli(
    *,
    ...
    active_matcher = matcher or build_matcher(
        debug=debug,
    ...
    output_func("=" * 54)
    output_func("  Welcome to Riverside Books CLI")
    output_func("  Ask me anything - type 'quit' or 'exit' to leave.")
    output_func("=" * 54)
    output_func(f"  Loaded {len(faqs)} FAQs")
    output_func(f"  Matcher mode: {'lexical only' if lexical_only else 'semantic'}")
    output_func("=" * 54)
...
def main(
    *,
    ...
    return run_cli(faq_path=faq_path, debug=debug, lexical_only=lexical_only)
