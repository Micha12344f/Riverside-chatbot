...
def _lexical_similarity(query: str, text: str) -> float:
    ...
    fuzzy = SequenceMatcher(None, _normalize_text(query), _normalize_text(text)).ratio()
...
class LexicalMatcher(Matcher):
    ...
    def match(self, query: str, faqs: list[FAQ]) -> MatchResult | None:
        ...
        candidates = tuple(
            sorted(
                (MatchCandidate(faq=faq, score=self._score(query, faq)) for faq in faqs),
...
class SemanticMatcher(Matcher):
    ...
    def _fallback_match(self, query: str, faqs: list[FAQ], exc: Exception) -> MatchResult | None:
        if not self._warned_about_fallback:
            self.logger("Bot: Embedding model unavailable, falling back to lexical matching.")
...
class LlmMatcher(Matcher):
    def match(self, query: str, faqs: list[FAQ]) -> MatchResult | None:
        raise NotImplementedError(
            "LlmMatcher is intentionally not implemented for this CLI app."
        )
